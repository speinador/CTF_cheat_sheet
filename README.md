...
Vulnerabilidades Básicas · Herramientas · Enfoque Práctico  
...
Índice  
1. Introducción  
2. Flujo de trabajo / Cómo encarar un reto  
...
10. Tabla de CVE comunes y fáciles  
...
- Herramientas típicas: `nmap -sC -sV -oA initial <IP>`, `gobuster dir`, `whatweb`.  
...
3.2 Explotación  
Para web:  
- `sqlmap -u "http://<IP>/vuln.php?id=1" --batch` — inyección SQL automatizada.  
...
- Mitigación: usar consultas preparadas, sanitizar/escapar inputs, aplicar mínimo privilegio en DB.
...
- Detección: parámetros tipo `?page=`, `?file=../../etc/passwd`; servicio web antiguo.  
- Ejemplo de explotación:  
  `?file=../../../../etc/passwd`  
  o  
  `?file=php://filter/convert.base64-encode/resource=index.php`  
- Mitigación: usar listas blancas de archivos permitidos, validar/sanitizar nombre del archivo, rutas absolutas seguras.
...
- Mitigación: aplicar mínimo privilegio, revisar servicios/configuraciones, mantener actualizado.
...
- Mitigación: deshabilitar listados de directorio, proteger backups, eliminar repositorios públicos no autorizados, cambiar credenciales por defecto.

4.6 Servicios Antiguos / Vulnerabilidades con CVE Públicas  
- Qué es: servicios desactualizados con vulnerabilidades conocidas.  
- Detección: Nmap detecta versión del servicio antigua. Buscar CVE asociadas.  
...
- Detección: campos que ejecutan comandos del sistema (`ping`, `download file`), tratar con `; ls /`, `| whoami`.  
- Ejemplo de explotación:  
  `; cat /etc/passwd`  
...
- Si un formulario no devuelve error pero el tiempo de respuesta cambia al enviar inyección, puede tratarse de una inyección SQL “time-based”.  
